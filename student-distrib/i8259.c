...
unsigned long flags;
	 ...
	 */
	outb_p(0x11, MASTER_8259_CMD_PORT);	/* ICW1: select 8259A-1 init */
    
	/* ICW2: 8259A-1 IR0-7 mapped to ISA_IRQ_VECTOR(0) */
	outb_p(ICW2_MASTER, MASTER_8259_IMR_PORT);

	/* 8259A-1 (the master) has a slave on IR2 */
	outb_p(0x04, PIC_MASTER_IMR_PORT);

	outb_pic(ICW4, MASTER_8259_IMR_PORT); /* master does normal eoi */

	outb_pic(0x11, SLAVE_8259_CMD_PORT);	/* ICW1: select 8259A-2 init */
...
void send_eoi(uint32_t irq_num) {
    if(irq_num & 8){
        if(irq_num >7){
            ...
            outb(EOI + 4, MASTER_8259_CMD_PORT);
        }
        else{
