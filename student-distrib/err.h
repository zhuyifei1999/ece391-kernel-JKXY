...
static inline __always_inline void * ERR_PTR(int32_t error) {
	return (void *) error;
}

static inline __always_inline int32_t PTR_ERR(const void *ptr) {
	return (int32_t) ptr;
}
...
static inline __always_inline bool IS_ERR_OR_NULL(const void *ptr) {
	return (!ptr) || IS_ERR_VALUE((uint32_t)ptr);
}

// cast away the pointer type in errors
static inline __always_inline void * ERR_CAST(const void *ptr) {
	/* cast away the const */
	return (void *) ptr;
}
